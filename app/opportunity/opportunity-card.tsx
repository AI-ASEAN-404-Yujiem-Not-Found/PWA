...
import { reduceWord } from "../helper/word-reducer";

export default function OpportunityCard({title, rec, detail}:{title: string; rec: string; detail:string}) {
    return (
        <div className='px-2 py-5 rounded-xl w-full border border-gray-200 shadow-lg space-y-3'>
            ...
            <h1 className='text-[20px] text-gray-700 font-semibold'>{title}</h1>
            <div className='bg-[#D3F4D7] py-2 px-4 rounded-xl flex items-center gap-2'>
                <FaCheck className='w-3 h-3 fill-[#2B9637]' />
                <p className='text-[10px] text-[#2B9637]'>{rec}</p>
            </div>
            <p className='text-[12px] text-gray-600 italic'>{reduceWord(detail, 60)}&quot;</p>
            <div className='w-full flex flex-col gap-3 justify-center'>
                <button className="p-3 w-full max-w-100 bg-[#004868] text-white rounded-md">
                    <Link href={"/additional-context"}>Apply Now</Link>
                </button>
